...
RAYGUIDEF bool GuiGetLocked(void);
RAYGUIDEF float GuiGetAlpha(void);

// Font set/get functions and Text Width
...
RAYGUIDEF int GetTextWidth(const char *text);                           // Gui get text width using default font
...
RAYGUIDEF void GuiDrawRectangle(Rectangle rec, int borderWidth, Color borderColor, Color color);        // Gui draw rectangle using default raygui style
RAYGUIDEF void GuiDrawText(const char *text, Rectangle bounds, int alignment, Color tint);              // Gui draw text using default font

// ---
RAYGUIDEF Rectangle GetTextBounds(int control, Rectangle bounds);                                       // Get text bounds considering control bounds
...
// Get gui locked (global guiLocked)
bool GuiGetLocked(void) {	return guiLocked; }

// Get gui alpha (global guiAlpha)
    ...
    if (tryBinary)
    {
            ...
            (signature[3] == ' '))
        {
            ...
            if (fontDataSize > 0)
            {
                ...
                printf("GuiLoadStyle font.chars %d\n", font.charsCount);
