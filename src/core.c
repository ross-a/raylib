...
typedef struct CoreData {
	...
	GLuint printBuffer;  // for printf from shaders
} CoreData;
...
// Setup printf from shaders
void BeginPrintf(void)
{
	// create a buffer to hold the printf results
	CORE.printBuffer = createPrintBuffer();
	// bind it to the current program
	bindPrintBuffer(RLGL.State.defaultShader.id, CORE.printBuffer);
}

// End printf from shaders
void EndPrintf(void)
{
	// convert to string, output to console
	printf("\n\nGLSL print: %s\n", getPrintBufferString(CORE.printBuffer));
	// clean up
	deletePrintBuffer(CORE.printBuffer);
}
...
static bool InitGraphicsDevice(int width, int height)
{
    ...
    else if (rlGetVersion() == OPENGL_46)
    {
